/**
 * Deploy EntropyLotto (Pyth Entropy adapter)
 *
 * Usage:
 *   npx hardhat run deploy/deploy-entropy-lotto.js --network base
 *
 * Required env:
...
async function main() {
  ...
  const EntropyLotto = await ethers.getContractFactory('EntropyLotto', deployer);
  const contract = await EntropyLotto.deploy(admin, entropyAddress, entropyProvider);
  await contract.waitForDeployment();

  const address = await contract.getAddress();
  console.log('EntropyLotto deployed:', address);
  ...
  console.log('\nSet env:');
  console.log(`ENTROPY_LOTTO=${address}`);
