/**
 * Deploy EntropySlots (Pyth Entropy adapter)
 *
 * Usage:
 *   npx hardhat run deploy/deploy-entropy-slots.js --network base
 *
 * Required env:
...
async function main() {
  ...
  const EntropySlots = await ethers.getContractFactory('EntropySlots', deployer);
  const contract = await EntropySlots.deploy(admin, entropyAddress, entropyProvider);
  await contract.waitForDeployment();

  const address = await contract.getAddress();
  console.log('EntropySlots deployed:', address);
  ...
  console.log('\nSet env:');
  console.log(`ENTROPY_SLOTS=${address}`);
